...
df = pd.read_csv('voyager2/voyager2_ephemeris.txt', skiprows = 57, skipfooter=216-165, engine='python')

df['date'] = df['date'].apply(lambda x: x[:12])

#set up ra and dec arrays
...
ax.set_title("Voyager 2 Path from Jan 1, 1978 to Jul 1, 2030")
ax.set_xlabel("RA")
ax.set_ylabel("Dec")
ax.set_facecolor('xkcd:light blue')
ax.grid(True)

plt.savefig('voyager2_equatorial_ephemeris.png')
plt.show()
...
ax2.set_title("Voyager 2 Path from Jan 1, 1978 to Jul 1, 2030 on Ecliptic Plane")
ax2.set_xlabel("Longitude")
ax2.set_ylabel("Latitude")
ax2.set_facecolor('xkcd:light blue')
ax2.grid(True)

plt.savefig('voyager2_ecliptic_ephemeris.png')
plt.show()